...
//注释
    ...
    ,releaseDir = '../pack/layuiDemo.pack/'+ pkg.name +'-v' + pkg.version //发行版本目录
...
//任务
    ...
    ,mv: function(){
        ...
        gulp.src('./src/lib/ztree/img/**/*')
            .pipe(gulp.dest(destDir + '/lib/ztree/img'));
...
//构建核心源文件
gulp.task('default',gulp.series( ['clear', 'src'], function(done){ //命令：gulp
    for(var key in task){
        task[key]();
    }
    done()
